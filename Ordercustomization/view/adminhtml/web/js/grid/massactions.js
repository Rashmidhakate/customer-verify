...
define([
    ...
    return Collapsible.extend({
        ...
        applyAction: function (actionIndex) {
			...
			var exportedTotal = 0;
			var num = 0;
			$(data.selected).each(function(index ,value) {
				if($.inArray(value, window.customInfo) != -1){
					added = true;
					exportedTotal += num;
					num++;
				}
			});
			if(action.type == 'exported'){
				if(added){
					this._confirm(action, callback, num)
				}else{
					callback();
				}
			}else{
         ...
         */
        _confirm: function (action, callback , exportTotal = '' ) {
            var confirmData = action.confirm,
                data = this.getSelections(),
                total = data.total ? data.total : 0;
            if(action.type == 'exported'){
				var message = ' (' + exportTotal + ' record' + (exportTotal > 1 ? 's' : '') + ')'+' already exported in sysytem, Do you want export again?';
				var confirmMessage = message + ' ( Total ' + total + ' record' + (total > 1 ? 's' : '') + ')';
            }else{
